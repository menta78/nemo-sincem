         ...
         DO ik = 1, jpkm1
            ...
            IF ( nint(zcoef1+zcoef2) == 0) THEN
               ...
               IF (zcoef > .5_wp) THEN ! Only set none isolated points.
                 tra(ii,ij,ik,jn) = tra(ii-1,ij  ,ik,jn) * pmask(ii-1,ij  ,ik) + &
                   &              tra(ii+1,ij  ,ik,jn) * pmask(ii+1,ij  ,ik) + &
                   &              tra(ii  ,ij-1,ik,jn) * pmask(ii  ,ij-1,ik) + &
                   &              tra(ii  ,ij+1,ik,jn) * pmask(ii  ,ij+1,ik)
                 tra(ii,ij,ik,jn) = ( tra(ii,ij,ik,jn) / zcoef ) * pmask(ii,ij,ik)
               ELSE
                 tra(ii,ij,ik,jn) =   tra(ii,ij,ik,jn) * pmask(ii,ij  ,ik)
               ENDIF
            ELSEIF ( nint(zcoef1+zcoef2) == 2) THEN
               ...
               tra(ii,ij,ik,jn) = tra(ii-1,ij  ,ik,jn) * pmask(ii-1,ij  ,ik)*bdypmask(ii-1,ij  ) + &
                   &            tra(ii+1,ij  ,ik,jn) * pmask(ii+1,ij  ,ik)*bdypmask(ii+1,ij  )  + &
                   &            tra(ii  ,ij-1,ik,jn) * pmask(ii  ,ij-1,ik)*bdypmask(ii,ij -1 ) + &
                   &            tra(ii  ,ij+1,ik,jn) * pmask(ii  ,ij+1,ik)*bdypmask(ii,ij+1  )

               tra(ii,ij,ik,jn) = ( tra(ii,ij,ik,jn) / MAX(1._wp, zcoef) ) * pmask(ii,ij,ik)
            ELSE
               ...
               tra(ii,ij,ik,jn) = tra(ii+ip,ij+jp,ik,jn) * pmask(ii+ip,ij+jp,ik)
            ENDIF
         END DO
      END DO
      !
